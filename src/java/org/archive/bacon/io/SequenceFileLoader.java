...
public class SequenceFileLoader extends FileInputLoadFunc {
  ...
  public boolean tagSource  = false;
  ...
  public SequenceFileLoader( String options )
  {
    this();
    this.tagSource = "-tagsource".equalsIgnoreCase( options );
  }
  ...
  public Tuple getNext() throws IOException {
    ...
    if ( this.tagSource ) 
      {
        mProtoTuple.add(new DataByteArray(sourcePath));
      }
  ...
  public void prepareToRead(RecordReader reader, PigSplit split)
    ...
    if ( tagSource )
      {
        sourcePath = ((FileSplit)split.getWrappedSplit()).getPath().getName();
      }
  }
